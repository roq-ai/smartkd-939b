...
import { getProblemFixes } from 'apiSdk/problem-fixes';
...
function FeedbackCreatePage() {
  ...
  const formik = useFormik<FeedbackInterface>({
    initialValues: {
      ...
      problem_fix_id: (router.query.problem_fix_id as string) ?? null,
    },
    ...
    <AppLayout>
      <Box bg="white" p={4} rounded="md" shadow="md">
        ...
        <form onSubmit={formik.handleSubmit}>
          ...
          />
          <AsyncSelect<ProblemFixInterface>
            formik={formik}
            name={'problem_fix_id'}
            label={'Select Problem Fix'}
            placeholder={'Select Problem Fix'}
            fetcher={getProblemFixes}
            renderOption={(record) => (
              <option key={record.id} value={record.id}>
                {record?.description}
              </option>
