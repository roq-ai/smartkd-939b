...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function createCompany() {
    ...
    const body = { ...req.body };
    if (body?.manual?.length > 0) {
      const create_manual = body.manual;
      body.manual = {
    ...
    if (body?.problem_fix?.length > 0) {
      const create_problem_fix = body.problem_fix;
      body.problem_fix = {
        create: create_problem_fix,
      };
    } else {
      delete body.problem_fix;
    }
